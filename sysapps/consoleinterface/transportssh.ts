            ...
            client
                .on('authentication', (ctx: AuthContext) => {
                    ...
                    switch (ctx.method) {
                        ...
                        case 'publickey':
                            let i;
                            for (i = 0; i < this._allowedPubKeys.length; i++) {
                                let allowedPubKey: string = this._allowedPubKeys[i].getPublicSSH();
                                if (ctx.key.algo == this._allowedPubKeys[i].type ||
                                    ctx.key.data.length === allowedPubKey.length ||
                                    Crypto.timingSafeEqual(ctx.key.data, Buffer.from(allowedPubKey)) ||
                                    (ctx.signature && this._allowedPubKeys[i].verify(ctx.blob, ctx.signature) == true)) {
                                    break;
                                }
                            }

                            if (i == this._allowedPubKeys.length) return ctx.reject();
